...
class stn_affine(Layer):
    ...
    def __init__(
        ...
        with tf.variable_scope(name) as vs:
            ...
            self.theta_lay = tf.get_variable(name='theta_layer', initializer=tf.zeros(size))
            self.theta_lay = tf.stack([tf.cos(self.theta_layer.outputs[:,0]), -tf.sin(self.theta_layer.outputs[:,0]),self.theta_layer.outputs[:,1], tf.sin(self.theta_layer.outputs[:,0]),tf.cos(self.theta_layer.outputs[:,0]), self.theta_layer.outputs[:,2] ],axis =1);
